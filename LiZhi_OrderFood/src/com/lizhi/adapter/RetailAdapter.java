...
public class RetailAdapter extends BaseAdapter {
	private RetailAdapterCallback callback;
	...
	public void setCallback(RetailAdapterCallback callback) {
		this.callback = callback;
	}

	public RetailAdapter(Context context, List<Merchandise> details) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = LayoutInflater.from(context).inflate(
					R.layout.adapter_retail_list, null);
			holder.id = (TextView) convertView.findViewById(R.id.tv_pro_id);
			holder.size = (TextView) convertView.findViewById(R.id.tv_pro_size);
			...
			holder.delete = (TextView) convertView
					.findViewById(R.id.tv_pro_del);

			holder.decrese = (ImageView) convertView
					.findViewById(R.id.iv_pro_decrease);
			holder.increase = (ImageView) convertView
					.findViewById(R.id.iv_pro_increase);
			 ...
			 * 
			 * convertView = LayoutInflater.from(context).inflate(
			 * R.layout.adapter_retail_list1, null); holder.barCode = (TextView)
			 * convertView .findViewById(R.id.tv_pro_barCode);
		...
		if (position < details.size()) {
			...
			final int Invertory = Integer.parseInt(retail.getInvertory());
			final double price = new BigDecimal(retail.getPrice()).setScale(2,
					BigDecimal.ROUND_HALF_UP).doubleValue();
			holder.id.setText((position + 1) + "");
			holder.name.setText(retail.getMerchandiseName());
			holder.price.setText(price + "");
			holder.size.setText("");// 规格
			...
			holder.delete.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					 ...
					 */
					callback.delete(
							Integer.parseInt(holder.id.getText().toString()) - 1,
							details);
				}
			});
			/*
			...
			holder.decrese.setOnClickListener(new OnClickListener() {
				...
				@Override
				public void onClick(View v) {
					int num = Integer.parseInt(holder.number.getText()
							.toString());
					// holder.number.getText();
					if (num > 1) {
						num--;
						holder.number.setText(num + "");
						/*
						 * double f1 = new BigDecimal(price * retail.getNum())
			...
			holder.increase.setOnClickListener(new OnClickListener() {
				...
				@Override
				public void onClick(View v) {
					// int num = retail.getNum();
					int num = Integer.parseInt(holder.number.getText()
							.toString());
					if (Integer.parseInt(holder.stock.getText().toString()) > 0) {
						num++;
						holder.number.setText(num + "");
						/*
						 * double f1 = new BigDecimal(price * retail.getNum())
			...
			holder.stock.setText(Integer.parseInt(retail.getInvertory()) - 1
					+ "");
			DecimalFormat df = new DecimalFormat("#.##");
			holder.total.setText(df.format(price * retail.getNum()));
			this.setTotlenum(totlenum + retail.getNum());
			/*
			 * } else {
			 * if(retail.getInvertory().equals("0")||retail.getClose().equals
			 * ("0")) holder.buy.setText("不可购买");
