...
public class HorizonListView extends AdapterView<ListAdapter> {
	...
	public HorizonListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		witdh = context.getResources().getDisplayMetrics().widthPixels;
		initView();
	}
	...
	private DataSetObserver mDataObserver = new DataSetObserver() { // 数据观察者/数据监听者
	...
	protected boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
		...
		synchronized (HorizonListView.this) {
			if (e2.getX() - e1.getX() > 40
					&& initPoint < getWidth() * getChildCount() / 2) {
				initPoint += witdh;
			}
			if (e1.getX() - e2.getX() > 40 && initPoint > 0) {
				initPoint -= witdh;
			}		
			// mScroller.fling(mNextX, 0, (int) -velocityX, 0, 0, mMaxX, 0, 0);
			mScroller.fling(initPoint, 0, (int) -velocityX, 0, 0, mMaxX, 0, 0);
		}
		requestLayout();
			...
			for (int i = 0; i < getChildCount(); i++) {
				...
				if (isEventWithinView(e, child)) {
					if (mOnItemClicked != null) {
						mOnItemClicked.onItemClick(HorizonListView.this, child,
								mLeftViewIndex + 1 + i,
				...
				if (isEventWithinView(e, child)) {
					if (mOnItemLongClicked != null) {
						mOnItemLongClicked.onItemLongClick(
								HorizonListView.this, child, mLeftViewIndex + 1
										+ i,
								mAdapter.getItemId(mLeftViewIndex + 1 + i));
					}
					break;
